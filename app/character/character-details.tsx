...
export const CharacterDetails = () => {
    ...
    return (
        <main className="flex flex-col pt-8 pb-4 max-w-prose md:mx-auto mx-4 gap-4">
            ...
            <section>
                <img className="py-4" src={`${result.thumbnail.path}.${result.thumbnail.extension}`} style={{ viewTransitionName: "image-expand" }}></img>
            ...
            <Table
                hideControls
                headers={["Comic Book"]}
                results={result.comics.items}
                total={result.comics.available}
                renderRow={(result) => (
                    <tr key={result.resourceURI} className="flex flex-col flex-1 gap-4 border-b md:gap-6 lg:gap-8 sm:table-row">
                ...
                renderHeader={(header) => <th className="p-2" key={header}>{header}</th>}
            />
            <footer>
